...
def start_game():
    try:
        ...
        word_pool = [word for cat in selected_categories for word in WORDS_DATABASE.get(cat, [])]
        if not word_pool: return redirect(url_for("setup_page"))
        
        secret_word = random.choice(word_pool)
        roles = ["جاسوس"] * num_spies + [secret_word] * (num_players - num_spies)
        random.shuffle(roles)

        # Create a list of player dictionaries and shuffle them to randomize roles
        players = [{"name": name, "role": None} for name in player_names]
...
def game_page():
    ...
    if player_index >= len(players):
        if session.get("game_mode") == "timed":
            return render_template("game.html", page_type="timer", time_limit=session.get("time_limit", 300), hourglass_svg=HOURGLASS_SVG)
        else:
            return render_template("game.html", page_type="final_message")
    else:
...
if __name__ == "__main__":
    print("="*50)
    print(">>> Spy Game PRO (v9 Player Names) در حال اجرا است...")
